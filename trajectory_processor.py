...
Supports any grid size with dynamic token allocation (no overlaps)
"""
...
class TrajectoryConfig:
    ...
    Supports both basic and enhanced modes with dynamic token allocation
    """
    ...
    # Mode
    enhanced_mode: bool = True  # Set to False for basic mode

    def __post_init__(self):
        """
        Compute token IDs dynamically based on grid_size
        This prevents token overlap issues when changing grid size
        """
        num_spatial = self.grid_size ** 2

        # Special tokens AFTER spatial (no overlap!)
        self.PAD_TOKEN = num_spatial      # 256 for 16×16, 1024 for 32×32
        self.START_TOKEN = num_spatial + 1
        self.END_TOKEN = num_spatial + 2

        # Scene context tokens
        self.SCENE_INTERSECTION = num_spatial + 3
        self.SCENE_HIGHWAY = num_spatial + 4
        self.SCENE_PARKING = num_spatial + 5
        self.SCENE_URBAN = num_spatial + 6

        # Lane context tokens
        self.LANE_STRAIGHT = num_spatial + 7
        self.LANE_LEFT = num_spatial + 8
        self.LANE_RIGHT = num_spatial + 9
        self.LANE_MERGE = num_spatial + 10

        # Traffic light tokens
        self.TRAFFIC_LIGHT_RED = num_spatial + 11
        self.TRAFFIC_LIGHT_YELLOW = num_spatial + 12
        self.TRAFFIC_LIGHT_GREEN = num_spatial + 13
        self.TRAFFIC_LIGHT_UNKNOWN = num_spatial + 14

        # Agent type tokens
        self.AGENT_CAR = num_spatial + 15
        self.AGENT_TRUCK = num_spatial + 16
        self.AGENT_BUS = num_spatial + 17

        # Agent marker
        self.NEARBY_AGENT_START = num_spatial + 18

        # Map context tokens
        self.ON_INTERSECTION = num_spatial + 19
        self.NEAR_CROSSWALK = num_spatial + 20
        self.NEAR_STOP_SIGN = num_spatial + 21

        # Vocabulary size: spatial + 3 special + 21 semantic
        self.vocab_size = num_spatial + 24

        # Build semantic token map dynamically
        self._build_semantic_map()

    def _build_semantic_map(self):
        """Build reverse lookup for semantic tokens"""
        self.semantic_token_map = {
            self.SCENE_INTERSECTION: "INTERSECTION",
            self.SCENE_HIGHWAY: "HIGHWAY",
            self.SCENE_PARKING: "PARKING",
            self.SCENE_URBAN: "URBAN",
            self.LANE_STRAIGHT: "STRAIGHT",
            self.LANE_LEFT: "LEFT",
            self.LANE_RIGHT: "RIGHT",
            self.LANE_MERGE: "MERGE",
            self.TRAFFIC_LIGHT_RED: "RED",
            self.TRAFFIC_LIGHT_YELLOW: "YELLOW",
            self.TRAFFIC_LIGHT_GREEN: "GREEN",
            self.TRAFFIC_LIGHT_UNKNOWN: "UNKNOWN",
            self.AGENT_CAR: "CAR",
            self.AGENT_TRUCK: "TRUCK",
            self.AGENT_BUS: "BUS",
            self.NEARBY_AGENT_START: "AGENT_START",
            self.ON_INTERSECTION: "ON_INTERSECTION",
            self.NEAR_CROSSWALK: "NEAR_CROSSWALK",
            self.NEAR_STOP_SIGN: "NEAR_STOP_SIGN"
        }
...
class GridDiscretizer:
    ...
    def discretize_point(self, x: float, y: float) -> int:
        ...
        Returns:
            token: Integer in [0, grid_size²-1]
        """
    ...
    def reconstruct_point(self, token: int) -> Tuple[float, float]:
        ...
        Args:
            token: Grid token [0, grid_size²-1]
    ...
    def reconstruct_trajectory(self, tokens: List[int]) -> np.ndarray:
        ...
        """
        trajectory = []
        num_spatial = self.config.grid_size ** 2  # Dynamic based on grid size

        for token in tokens:
            # Only reconstruct spatial tokens (0 to num_spatial-1)
            if 0 <= token < num_spatial:
...
class TrajectoryProcessor:
    ...
    def process_basic_trajectory(
        ...
        """
        Basic trajectory processing (original mode)
    ...
    def decode_semantic_tokens(self, tokens: List[int]) -> Dict:
        ...
        semantic_map = self.config.semantic_token_map

        for token in tokens:
            token_name = semantic_map.get(token, "")

            if token_name in ["INTERSECTION", "HIGHWAY", "PARKING", "URBAN"]:
                scene_type = token_name
            elif token_name in ["STRAIGHT", "LEFT", "RIGHT", "MERGE"]:
                lane_context.append(token_name)
            elif token_name in ["ON_INTERSECTION", "NEAR_CROSSWALK", "NEAR_STOP_SIGN"]:
                map_features.append(token_name)
            elif token_name in ["CAR", "TRUCK", "BUS"]:
                agents.append(token_name)
    ...
    def get_token_name(self, token: int) -> str:
        ...
        num_spatial = self.config.grid_size ** 2

        if token < num_spatial:
            return f"SPATIAL_{token}"
        elif token == self.config.PAD_TOKEN:
            return "PAD"
        elif token == self.config.START_TOKEN:
            return "START"
        elif token == self.config.END_TOKEN:
            return "END"
        elif token in self.config.semantic_token_map:
            return self.config.semantic_token_map[token]
        else:
...
def create_basic_processor(grid_size: int = 16) -> TrajectoryProcessor:
    """Create processor for basic mode"""
    config = TrajectoryConfig(
        grid_size=grid_size,
        cell_size=32.0 / grid_size,  # Maintain 32m coverage
        enhanced_mode=False
    )
    return TrajectoryProcessor(config)


def create_enhanced_processor(grid_size: int = 16) -> TrajectoryProcessor:
    """Create processor for enhanced mode"""
    config = TrajectoryConfig(
        grid_size=grid_size,
        cell_size=32.0 / grid_size,  # Maintain 32m coverage
        enhanced_mode=True
...
def demo_processor():
    """Demonstrate both modes and grid sizes"""
    print("TrajectoryProcessor v2 Demo (Dynamic Tokens)")
    print("=" * 60)
    ...
    for grid_size in [16, 32]:
        ...
        processor = create_enhanced_processor(grid_size=grid_size)

        print(f"Spatial tokens: 0-{grid_size**2-1}")
        print(f"PAD: {processor.config.PAD_TOKEN}")
        print(f"START: {processor.config.START_TOKEN}")
        print(f"END: {processor.config.END_TOKEN}")
        print(f"First semantic: {processor.config.SCENE_INTERSECTION}")
        print(f"Vocab size: {processor.config.vocab_size}")
        ...
        print(f"Target tokens: {len(result['target_tokens'])}")

    print("\n" + "=" * 60)
    print("✓ Dynamic token allocation working correctly!")
    print("✓ No token overlaps between spatial and semantic!")
